...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        let gameInfo = `<img class="game-img" src=${games[i].img} alt="${name} Image" />
                        <p><strong>${games[i].name} </strong></p>
                        <p style="color: gray;">${games[i].description}</p>
                        <p>Total raised: $${games[i].pledged.toLocaleString("es-US")}</p>
                        <p>Goal: $${games[i].goal.toLocaleString("es-US")}</p>
                        <p>Unique donators: ${games[i].backers.toLocaleString("es-US")}</p>`
...
// displaying all games in games container
...
const countUnfundedGames = GAMES_JSON.reduce((acc, game) => {
    // increment acc by 1 if the game is unfunded
    if (game.goal > game.pledged) return acc + 1;

    return acc;
}, 0)
// create a string that explains the number of unfunded games using the ternary operator
let descriptionStr = `A total of $${totalRaised.toLocaleString("en-US")} has been raised 
for ${totalGames} games. Currently, ${countUnfundedGames}${countUnfundedGames > 1 ? " games remain" : " game remains"} 
